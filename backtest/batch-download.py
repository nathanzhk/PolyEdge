...
def main():
    if len(sys.argv) != 3:
        print("Usage: python batch-download.py <start-date> <end-date>")
        print("Example: python batch-download.py 2026-03-09 2026-04-07")
        sys.exit(1)
    ...
    for i, d in enumerate(all_dates, 1):
        ...
        if not binance_file.exists():
            r = subprocess.run(
                ["python3", str(DATA_DIR / "download-binance.py"), d],
                capture_output=True,
                text=True,
            )
            if r.returncode != 0:
        ...
        if not market_file.exists():
            r = subprocess.run(
                ["python3", str(DATA_DIR / "download-market.py"), d],
                capture_output=True,
                text=True,
            )
            if r.returncode != 0:
